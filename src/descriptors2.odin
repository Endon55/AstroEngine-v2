...
descriptor_growable_init::proc(self:^Descriptor_Allocator_Growable, device: vk.Device, max_sets:u32, pool_ratios:[]Pool_Size_Ratio,) ->(ok:bool,) {
...
descriptor_growable_allocate :: proc(self:^Descriptor_Allocator_Growable, layout: ^vk.DescriptorSetLayout, pNext: rawptr = nil,) -> (ds: vk.DescriptorSet, ok: bool,) {
...
descriptor_growable_get_pool :: proc(self:^Descriptor_Allocator_Growable,) ->(pool:vk.DescriptorPool, ok:bool,) {
...
descriptor_growable_create_pool :: proc(self:^Descriptor_Allocator_Growable,set_count: u32) ->(pool:vk.DescriptorPool, ok:bool,) {
...
descriptor_writer_write_image :: proc(
    ...
    append(&self.image_infos, vk.DescriptorImageInfo{
        sampler = sampler,
        imageView = image,
        imageLayout = layout
    })
...
descriptor_writer_write_buffer :: proc(
    ...
    append(&self.buffer_infos, vk.DescriptorBufferInfo {
        buffer = buffer,
        offset = offset,
        range = size,
    })
...
descriptor_writer_update_set :: proc(self: ^Descriptor_Writer, set: vk.DescriptorSet, loc:= #caller_location) {
    ...
    if len(self.writes) > 0 {
        vk.UpdateDescriptorSets(self.device, u32(len(self.writes)), raw_data(self.writes[:]), 0, nil)
    }
